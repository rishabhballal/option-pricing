...
class Stock:
    def __init__(self, count=10**4, spot=100, rate=0.05, divid=0.03, vol=0.10):
        self.count = count
        self.spot = spot
        self.rate = rate
        self.divid = divid
    ...
    def geom_brownian(self, time=1, steps=10**3, rand=[]):
        if not len(rand):
        ...
        def wrapper(self, steps_=10**2, rand=[]):
            if not len(rand):
                rand = rng.standard_normal((steps_ * len(self.times), \
                    self.stock.count))
            return func(self, steps_ * len(self.times), rand)
        return wrapper

    @_random_seed
    def price(self, total_steps, rand):
